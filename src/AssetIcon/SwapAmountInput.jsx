...
const SwapAmountInput = ({
    ...
    return (
        <div className="flex flex-col mb-4">
            <div className="flex justify-between items-center border border-gray-300 rounded-md focus:outline-none p-2">
                <input
                    type="text"
                    value={amount}
                    onChange={(e) => onAmountChange(e.target.value)}
                    className="w-full py-2 focus-visible:outline-none"
                    placeholder={label}
                    data-testid={`amount-input-${label.toLowerCase()}`}
                />
                <button
                    onClick={openModal}
                    className="ml-2 min-w-fit px-2 py-1 bg-blue-500 text-white rounded-md"
                >
